...
class VQ(nn.Module):
    ...
    def encode(self, x, input_chans=None, input_time=None, mask=None):
        ...
        encoder_features = self.encoder(
            x, input_chans, input_time, mask, return_all_tokens=True
        )
    ...
    def decode(self, quantize, input_chans=None, input_time=None, mask=None, **kwargs):
        ...
        decoder_features_freq = self.decoder_freq(
            quantize, input_chans, input_time, mask, return_all_tokens=True
        )
        decoder_features_raw = self.decoder_raw(
            quantize, input_chans, input_time, mask, return_all_tokens=True
        )
    ...
    def get_codebook_indices(
        self, x, input_chans=None, input_time=None, input_mask=None, **kwargs
    ):
...
class VQ_Align(nn.Module):
    def __init__(
        ...
        self.domain_classifier = nn.Sequential(
            nn.Linear(decoder_config.n_embd, 256), nn.GELU(), nn.Linear(256, 2)
        )
    ...
    def forward(
        ...
        if y_freq is not None:
            loss, encoder_features, log = self.VQ(
            ...
            target = torch.full(
                (domain_out.size(0), domain_out.size(1)), fill_value=-1, device=x.device
            )
            target[input_mask] = 0
            domain_loss = F.cross_entropy(
                domain_out.view(-1, domain_out.size(-1)),
                target.view(-1),
                ignore_index=-1,
            )
            split = "train" if self.training else "val"
            ...
            domain_loss = F.cross_entropy(
                domain_out.view(-1, domain_out.size(-1)),
                torch.ones((x.size(0) * x.size(1),), device=x.device).long(),
                ignore_index=-1,
            )
            return domain_loss

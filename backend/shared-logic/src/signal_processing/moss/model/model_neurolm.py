...
class NeuroLM(nn.Module):
    def __init__(
        ...
        if tokenizer_ckpt_path is not None:
            ...
            for k, v in list(tokenizer_state_dict.items()):
                if k.startswith(unwanted_prefix):
                    tokenizer_state_dict[k[len(unwanted_prefix) :]] = (
                        tokenizer_state_dict.pop(k)
                    )
        ...
        # task layer
        self.encode_transform_layer = (
            nn.Sequential(
                nn.Linear(n_embd, self.GPT2.config.n_embd),
                nn.GELU(),
            )
            if n_embd != self.GPT2.config.n_embd
            else nn.Identity()
        )
    ...
    def forward(
        ...
        if x_eeg is not None:
            input_mask = (
                input_mask.unsqueeze(1).repeat(1, x_eeg.size(1), 1).unsqueeze(1)
            )
            x_eeg = self.tokenizer(
                x_eeg, input_chans, input_time, input_mask, return_all_tokens=True
            )
            x_eeg = self.encode_transform_layer(x_eeg)
            x_eeg += self.pos_embed(input_chans)

        logits, loss, accuracy = self.GPT2(
            x_eeg, y_eeg, x_text, y_text, input_time, eeg_mask, eeg_text_mask
        )
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            ...
            if eeg_text_mask is not None:
                eeg_text_mask = torch.cat(
                    (
                        eeg_text_mask,
                        torch.zeros(
                            (
                                eeg_text_mask.size(0),
                                eeg_text_mask.size(1),
                                eeg_text_mask.size(2),
                                1,
                            ),
                            device=eeg_text_mask.device,
                        ),
                    ),
                    dim=-1,
                )

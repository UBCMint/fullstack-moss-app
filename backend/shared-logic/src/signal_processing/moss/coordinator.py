...
Architecture:
    ...
    coordinator.py          <- you are here
        |           |               |
    preprocessing  encoder    classifier
"""
...
from preprocessing import preprocess, from_array
...
def _build_result(segments, src_fs, duration, task) -> dict:
    ...
    class_probabilities = {
        ...
        for name, p in sorted(zip(clf.label_names, mean_proba), key=lambda x: -x[1])
    }
...
def predict(input_path: str, task: str) -> dict:
    """
    Full prediction pipeline from a CSV file.
    ...
    return _build_result(segments, src_fs, duration, task)
...
def predict_from_array(raw: np.ndarray, src_fs: int, task: str) -> dict:
    ...
    Example:
        raw = np.array(lsl_samples, dtype=np.float32)

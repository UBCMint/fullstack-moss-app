...
def load_neurolm():
    checkpoint_path = os.path.join(
        NEUROLM_DIR, "checkpoints", "checkpoints", "NeuroLM-B.pt"
    )
...
def load_recording(filepath):
    ...
    if "RAW_TP9" in cols:
        ...
        eeg = (
            df[["channel1", "channel2", "channel3", "channel4"]]
            .dropna()
            .values.astype(np.float32)
        )
    else:
        raise ValueError(
            f"Unrecognized columns. Expected RAW_TP9/AF7/AF8/TP10 or channel1-4. Got: {cols[:10]}"
        )
    ...
    resampled = np.stack(
        [scipy_signal.resample(eeg[:, c], n_out) for c in range(4)], axis=1
    )
...
def segment_to_tensors(seg):
    ...
    input_time = [i for i in range(n_time) for _ in range(n_chans)] + [0] * (
        EEG_MAX_LEN - valid_len
    )
    input_time = torch.IntTensor(input_time)
    return (
        X_eeg.unsqueeze(0),
        input_chans.unsqueeze(0),
        input_time.unsqueeze(0),
        eeg_mask.bool().unsqueeze(0),
    )
...
def embed_segment(model, seg):
    ...
    tokens = model.tokenizer(
        X_eeg, input_chans, input_time, mask, return_all_tokens=True
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="MOSS - Predict mental state from Muse 2 EEG"
    )
    parser.add_argument(
        "--input", required=True, help="Path to your Muse 2 CSV recording"
    )
    parser.add_argument(
        "--task", default="activity", help="Task to run (default: activity)"
    )
